  ...
  return (
    <MotionDiv delay={delay}>
      <Link href={href} onClick={() => sessionStorage.setItem("mp-catalog-scroll", String(window.scrollY))} className="group block" aria-label={`View ${product.name}`}>
        {/* Image container — increased height, magnifies on hover */}
        {product.image ? (
          <ZoomImage
            src={product.image}
            alt={product.name}
            sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
            className="rounded-xl mb-4 transition-transform duration-500 group-hover:scale-[1.015]"
          ...
          <div
            className="relative rounded-xl overflow-hidden mb-4 flex flex-col items-center justify-center gap-2 bg-gray-100 text-gray-400"
            style={{ aspectRatio: "4 / 4.2" }}
          >
