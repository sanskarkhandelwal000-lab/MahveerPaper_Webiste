  ...
  return (
    <MotionDiv delay={delay}>
      <Link href={href} onClick={() => sessionStorage.setItem("mp-catalog-scroll", String(window.scrollY))} className="group block" aria-label={`View ${product.name}`}>
        {/* Image container — increased height. No hover-zoom here by design: this is
            the /products family/book listing grid (Spectrum, Textures, …), which
            just links through to the product page — zoom lives on the swatch grid
